...
import { ClientHttp2Session, Http2Stream } from 'http2'
import { KsqlRequest } from './api/ksql/ksql-request'
...
export class KsqlDBClient {
  ...
  public async listStreamsExtended(
    props?: Omit<KsqlRequest, 'ksql'>
  ): Promise<StreamsResponseExtended> {
    return this.statement<StreamsResponseExtended>('LIST STREAMS EXTENDED;', props).then(
      (res) => res[0]
    )
  }
  ...
  public async listTablesExtended(
    props?: Omit<KsqlRequest, 'ksql'>
  ): Promise<TablesResponseExtended> {
    return this.statement<TablesResponseExtended>('LIST TABLES EXTENDED;', props).then(
      (res) => res[0]
    )
  }
  ...
  queryStream<T>(body: QueryStreamRequest): Observable<T> {
    ...
    req
      ...
      .on('data', (data: any) => {
        if (!header) {
          // the first line in the response is the header
          header = data
        } else {
          const transform = header.columnNames.reduce(
